...
describe Article do
  ...
  describe "merge_with method" do
    before :each do 
      @article=Factory.create(:article, :id=> 3, :body=> 'content1', :title=>'title1', :author=> 'ben2')
      @other_article=Factory.create(:article, :id=> 4, :body=> 'content2',:title => 'title2', :author => 'drew3')
      Factory(:comment, :article=>@article)
      Factory(:comment, :article=>@other_article)
    end

    subject { @article.merge_with(@other_article) }

    it "should contain the content of both articles" do
      subject.body.should include(@article.body, @other_article.body)
    end
    ...
    it "should contain comments of both articles" do
      subject.comments.should include(*@article.comments, *@other_article.comments)
    end
